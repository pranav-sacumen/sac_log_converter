...
import configparser  # pylint: disable=C0301
import os
import shutil
...
def set_doc_config() -> None:
    ...
    for index, content in enumerate(contents):
        if "extensions = [" in content:
            ...
            contents.insert(
                index,
                "extensions = ['sphinxcontrib.fulltoc', 'sphinx.ext.autodoc', 'pallets_sphinx_themes']\n"
            )
...
def update_index_rst(project_path: str, config: Dict[str, Any]) -> None:
    ...
    run(f'sphinx-apidoc --module-first -M -o  . {module_path}')
...
def remove_heading(config: Dict[str, Any]) -> None:
    ...
    for sub_package in sub_package_list:
        ...
        if os.path.exists(rst_filename):
            with open(rst_filename) as rst_file:
    ...
    for sub_package in sub_package_list:
        ...
        while count < len(sub_modules):
            if sub_package_list[0] == sub_package:
                contents = update_content_with_automodule(
                    contents, sub_modules, count)
                contents.pop(0)
                contents.pop(0)
            else:
                try:
...
def generate_documentation(
        project_name: str, config_dirname: str, config_filename: str) -> None:
    """
...
if __name__ == '__main__':
    generate_documentation("Connectors", "config", "documentation.cfg")
